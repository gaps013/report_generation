...
class ReportGeneration(nn.Module):
    def __init__(self, device, visual_feature_size, features_extracted=True, fine_tuning=False,
        ...
        self.image_feature_projection = nn.Linear(self.visual_feature_size, self.embedding_dim)
    ...
    def forward(self, inputs):
        ...
        image_features = image_features.view(inputs["images"].size(0), self.visual_feature_size, -1).permute(0, 2, 1)
        ...
        if(not inputs['inference']):
            ...
            output_logits = self.combined_model(projected_image_features,
                                                tokenised_report, report_length)
